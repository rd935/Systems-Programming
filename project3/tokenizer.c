...
void tokenExpansion(arrlist* arr, char* token)
{
	...
	//now we push the array elements of argv to our arrlist arr
...
int tokenizer(arrlist* arr, char* line)
{
	...
	while(i <= lineSize)
	{
	       	...
	       	else if(line[i] == ' ' && line[left] != ' ')
		{
			...
			//we will see if the token has a wildcard symbol 
                        ...
                        else if(isWildCard(str) == 1 && isPrevTokenRedirect == 0) 
                        {
                                tokenExpansion(arr, str);
				isPrevWildCard = 1;
                        }
                        else if(isWildCard(str) == 1 && isPrevTokenRedirect == 1) 
                        {
